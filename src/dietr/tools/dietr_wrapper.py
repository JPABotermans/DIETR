...
from dietr.tools.validation import decode_y_prd_batch
...
class DIETRWrapper:
    def __init__(self, conf_pth: str, ckpt_pth: str, device: str = "cuda:0") -> None:
        with open(conf_pth, "r") as file:
            self.config = yaml.safe_load(file)
        self.device = device
        self.dietr = setup_dietr(config=self.config, device=self.device)
        ckpt = torch.load(ckpt_pth, map_location=self.device)
        if self.config["compiling"]:
            self.dietr = torch.compile(self.dietr)
        self.dietr.load_state_dict(ckpt["model"])
